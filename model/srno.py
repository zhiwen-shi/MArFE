    ...
    def query_rgb(self, coord, cell, input):
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                rel_coord[:, 1, :, :] *= feat.shape[-1] #16, 2, h, w
        ...
        grid = torch.cat([*rel_coords, *preds, \
            rel_cell.unsqueeze(-1).unsqueeze(-1).repeat(1,1,coord.shape[1],coord.shape[2])],dim=1)  # 16, c, h, w
        ...
        feat = x
        ret = self.fc2(F.gelu(self.fc1(feat))) # 16, 2, h, w
        ret = ret + F.grid_sample(input, coord.flip(-1), mode='bilinear',\
...
#     args = Args()
#     model = SRNO(args)  # 2x or 4x or 6x
