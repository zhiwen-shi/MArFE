...
class WKLoss(nn.Module):
    ...
    def forward(self, sr, FSsr, hr, shape1, shape2, s):
        b,c,h,w = hr.shape
        ...
        loss = torch.einsum('ij,jdhw->idhw', s, torch.abs((FSsr-FShr)*mask))
        loss = torch.mean(loss)     
        return loss

# if __name__ == "__main__":
#     model_test = KLoss(0)  #占位符没有实际意义
#     a = torch.randn([5,2,20,20])
#     b = torch.randn([5,2,20,20]) #具体的损失函数测试过程
#     loss = model_test(a,None,b,5,5) #输入了一个FSsr
#     print(loss)
